...
if __name__ == "__main__":
    ...
    train = df_normal[:train_len-dirty_len]  # sequences of normal or abnormal
    train = train.append(df_abnormal[:dirty_len])

    labels = np.append(np.zeros(train_len-dirty_len), np.ones(dirty_len)).astype(int)

    train = train.sample(frac = 1, random_state=12)  # shuffle the sequences

    # deeplog_file_generator(os.path.join(output_dir,'train'), train, ["EventId", "deltaT"])
    deeplog_file_generator(os.path.join(output_dir,'train_small'), train, ["EventId"])
    ...
    # Test Normal #
    ###############
    # test_normal = df_normal[train_len-dirty_len:]
    # nor_labels = np.zeros_like(test_normal)
    #
    ...
    # deeplog_file_generator(os.path.join(output_dir,'test_abnormal'), df_abnormal[dirty_len:], ["EventId"])
    # print('test abnormal size {}'.format(len(df_abnormal)-dirty_len))
