...
def sliding_window(raw_data, para):
    ...
    logkey_data, deltaT_data = raw_data.iloc[:, 2], raw_data.iloc[:, 3]   # time delta
    new_data = []
    ...
    for (start_index, end_index) in start_end_index_pair:
        ...
        new_data.append([
            ...
            max(label_data[start_index:end_index]),  # if it contains an abnormal event, it is an abnormal sequence
    ...
    return pd.DataFrame(new_data, columns=["timestamp", "Label", "tokenLabel", "EventId", "deltaT"])
