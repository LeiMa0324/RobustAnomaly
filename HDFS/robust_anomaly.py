...
options["output_dir"] = "../output/hdfs/dirty_0.05/"
...
options["min_len"] = 10
options["mask_ratio"] = 0.20   #masking rate
options["is_robust"] = True
...
options["is_time"] = False   #if train with time stamp
...
options["epochs"] = 200
options["n_epochs_stop"] = 10  # the number of early stop for no improvment epochs
...
if __name__ == "__main__":
    ...
    if args.mode == 'train':
        ...
        print(f"{model_dir} prediction finished!\n")

    elif args.mode == 'vocab':
        with open(options["train_vocab"], "r", encoding=args.encoding) as f:
            texts = f.readlines()
        vocab = WordVocab(texts, max_size=args.vocab_size, min_freq=args.min_freq, if_time=True)
